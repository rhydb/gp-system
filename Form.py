...
class Form(Frame):
    def __init__(self, *args, data={}, display="block", **kwargs):
        ...
        for j, item in enumerate(data): # data is an array of dictionaries
            ...
            if item.get("type") == "entry":
                ...
                OptionMenu(self, self.form_items[item.get("name")], *item.get("menu_items")).grid(row=(j+1 if display == "block" else 1), column=(1 if display == "block" else j))
